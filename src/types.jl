...
"""
mutable struct NoiseProcess{T,N,Tt,T2,T3,ZType,F,F2,inplace,S1,S2,RSWM,C,RNGType} <:
...
(W::NoiseWrapper)(out1, out2, u, p, t) =
...
mutable struct NoiseFunction{T,N,wType,zType,Tt,T2,T3,inplace} <:
    ...
    function NoiseFunction{iip}(
        ...
        new{
            typeof(noise_prototype),
            ndims(noise_prototype),
            typeof(W),
            typeof(Z),
            typeof(curt),
            typeof(curW),
...
function VirtualBrownianTree!(
    ...
    W0,
    Z0 = nothing,
    dist = INPLACE_WHITE_NOISE_DIST,
    bridge = INPLACE_VBT_BRIDGE;
    kwargs...,
)
...
function BoxWedgeTail{iip}(
    ...
    if box_grouping == :MinEntropy
        box, probability, offset =
            generate_boxes2(jpdf, Δr, Δa, Δz, one(Δr), one(Δa), one(Δz) / 64, rM, aM)
        dist_box = Distributions.Categorical(probability / sum(probability))
        boxes =
            BoxGeneration2{typeof(box),typeof(probability),typeof(offset),typeof(dist_box)}(
                box,
                probability,
                offset,
                dist_box,
    ...
    elseif box_grouping == :Columns
        ...
        boxes =
            BoxGeneration1{typeof(box),typeof(probability),typeof(offset),typeof(dist_box)}(
                box,
                probability,
                offset,
                dist_box,
    ...
    elseif box_grouping == :none
        ...
        dist_box = Distributions.Categorical(probability / val1)
        boxes =
            BoxGeneration3{typeof(box),typeof(probability),typeof(offset),typeof(dist_box)}(
                box,
                probability,
                offset,
                dist_box,
    ...
    dist_box = Distributions.Categorical(probability / sum(probability))
    wedges = Wedges{typeof(box),typeof(probability),typeof(offset),typeof(dist_box)}(
        box,
        probability,
        offset,
        dist_box,
    ...
    if nr == 4 && na == 4 && nz == 10
        ...
        error(
            "Cubature not implemented but needed for a different discretization. Please report this error.",
        )
    end
...
function BoxWedgeTail!(
    ...
    Z0 = nothing,
    dist = INPLACE_WHITE_NOISE_DIST,
    bridge = INPLACE_WHITE_NOISE_BRIDGE;
    kwargs...,
)
