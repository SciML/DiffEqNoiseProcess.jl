...
function interpolate!(W::VirtualBrownianTree, u, p, t)
    ...
    elseif ts[i - 1] == t # Can happen if it's the first value!
        ...
        if W.dZ !== nothing
            val1,
                val2 = search_VBT(
                t, W.seeds[i - 1], ts[i - 1], ts[i], timeseries[i - 1],
                timeseries[i],
...
function split_VBT_seed(rng, parent_seed, current_depth, Nt)
...
function search_VBT(
    ...
    while (abs(t - tmid) > W.atol && depth < W.search_depth)
        ...
        if Z0 !== nothing
            out2 = W.bridge(
                W.dW, nothing, Z0tmp, Z1tmp, q, h, nothing, nothing, nothing,
                rng
            )
        end
    end
...
function search_VBT!(
    ...
    while (abs(t - tmid) > W.atol && depth < W.search_depth)
        ...
        W.bridge(
            out1, W.dW, nothing, W.W0tmp, W.W1tmp, q, h, nothing, nothing, nothing,
            rng
        )
        if Z0 !== nothing
            W.bridge(
