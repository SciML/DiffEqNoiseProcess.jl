...
function interpolate!(W::VirtualBrownianTree,u,p,t)
  ...
  sign(W.dt)*t > sign(W.dt)*ts[end] && error("Bridge cannot extrapolate past the final timepoint. Build a longer VBT to cover the integration.")
  sign(W.dt)*t < sign(W.dt)*ts[1] && error("Bridge cannot extrapolate before the first timepoint. Build a longer VBT to cover the integration.")
  ...
  elseif ts[i-1] == t # Can happen if it's the first value!
    ...
    if W.dZ != nothing
      val1, val2 = search_VBT(t,W.seeds[i-1],ts[i-1],ts[i],timeseries[i-1],timeseries[i],
...
function interpolate!(out1,out2,W::VirtualBrownianTree,u,p,t)
  ...
  elseif ts[i-1] == t # Can happen if it's the first value!
    ...
    if W.dZ != nothing
      search_VBT!(out1,out2,t,seeds[i-1],ts[i-1],ts[i],timeseries[i-1],timeseries[i],
        timeseries2[i-1],timeseries2[i], W, W.rng)
    else
      search_VBT!(out1,out2,t,seeds[i-1],ts[i-1],ts[i],timeseries[i-1],timeseries[i],
        nothing,nothing, W, W.rng)
...
function create_VBT_cache(bridge,t0,W0,Z0,tend,Wend,Zend,rng::Random123.AbstractR123,tree_depth,search_depth)
...
function search_VBT(t,seed,t0,t1,W0,W1,Z0,Z1,W::VirtualBrownianTree,rng::Random123.AbstractR123)
  ...
  while (abs(t-tmid) > W.atol && depth<W.search_depth)
    ...
    if Z0 != nothing
      out2 = W.bridge(W.dW,nothing,Z0tmp,Z1tmp,q,h,nothing,nothing,nothing,rng)
    end
  end
...
function search_VBT!(out1,out2,t,seed,t0,t1,W0,W1,Z0,Z1,W::VirtualBrownianTree,rng::Random123.AbstractR123)
